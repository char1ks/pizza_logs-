...
<<<<<<< HEAD
        ...
        self.logger.info(
            "🍕 ЗАКАЗ ПИЦЦЫ: Получено событие OrderCreated в Notification Service",
            order_id=order_id,
            correlation_id=correlation_id,
            user_id=user_id,
            stage="notification_order_created_received",
        ...
        if not order_id:
            self.logger.warning(
                "🍕 ЗАКАЗ ПИЦЦЫ: OrderCreated событие без orderId",
                correlation_id=correlation_id,
                stage="notification_order_created_validation_failed",
                service="notification-service",
        ...
        self.logger.info(
            "🍕 ЗАКАЗ ПИЦЦЫ: Получаем шаблон уведомления для OrderCreated",
            order_id=order_id,
            correlation_id=correlation_id,
            stage="notification_template_fetch",
            service="notification-service"
        )
        ...
        if not template:
            self.logger.error(
                "🍕 ЗАКАЗ ПИЦЦЫ: Шаблон OrderCreated не найден",
                order_id=order_id,
                correlation_id=correlation_id,
                stage="notification_template_not_found",
                service="notification-service"
            )
            return

        self.logger.info(
            "🍕 ЗАКАЗ ПИЦЦЫ: Формируем сообщение уведомления из шаблона",
            order_id=order_id,
            correlation_id=correlation_id,
            stage="notification_message_formatting",
            service="notification-service"
        )
        ...
        self.logger.info(
            ...
            correlation_id=correlation_id,
            user_id=user_id,
            stage="notification_saving_to_db",
            service="notification-service"
        )
...
<<<<<<< HEAD
        ...
        if notification_id:
            self.logger.info(
                "🍕 ЗАКАЗ ПИЦЦЫ: Уведомление о создании заказа успешно обработано",
                order_id=order_id,
                correlation_id=correlation_id,
                notification_id=notification_id,
                stage="notification_order_created_completed",
                service="notification-service"
            )
        else:
            self.logger.error(
                "🍕 ЗАКАЗ ПИЦЦЫ: Ошибка при сохранении уведомления о создании заказа",
                order_id=order_id,
                correlation_id=correlation_id,
...
<<<<<<< HEAD
        ...
        self.logger.info(
            "🍕 ЗАКАЗ ПИЦЦЫ: Получено событие OrderPaid в Notification Service",
            order_id=order_id,
            correlation_id=correlation_id,
            user_id=user_id,
            stage="notification_order_paid_received",
            service="notification-service"
        )
        
        if not order_id:
            self.logger.warning(
                "🍕 ЗАКАЗ ПИЦЦЫ: OrderPaid событие без order_id",
                correlation_id=correlation_id,
                stage="notification_order_paid_validation_failed",
                service="notification-service",
                event_data=event_data
            )
            return
        ...
        self.logger.info(
            "🍕 ЗАКАЗ ПИЦЦЫ: Получаем шаблон уведомления для OrderPaid",
            order_id=order_id,
            correlation_id=correlation_id,
            stage="notification_template_fetch",
            service="notification-service"
        ...
        if not template:
            self.logger.error(
                "🍕 ЗАКАЗ ПИЦЦЫ: Шаблон OrderPaid не найден",
                order_id=order_id,
                correlation_id=correlation_id,
                stage="notification_template_not_found",
                service="notification-service"
        ...
        self.logger.info(
            "🍕 ЗАКАЗ ПИЦЦЫ: Формируем сообщение уведомления об успешном платеже",
            order_id=order_id,
            correlation_id=correlation_id,
            stage="notification_message_formatting",
            service="notification-service"
...
<<<<<<< HEAD
        ...
        if notification_id:
            self.logger.info(
                "🍕 ЗАКАЗ ПИЦЦЫ: Уведомление об успешном платеже обработано",
                order_id=order_id,
                correlation_id=correlation_id,
                notification_id=notification_id,
                stage="notification_order_paid_completed",
                service="notification-service"
            )
        else:
            self.logger.error(
                "🍕 ЗАКАЗ ПИЦЦЫ: Ошибка при сохранении уведомления об успешном платеже",
                order_id=order_id,
                correlation_id=correlation_id,
...
<<<<<<< HEAD
        ...
        self.logger.info(
            "🍕 ЗАКАЗ ПИЦЦЫ: Получено событие PaymentFailed в Notification Service",
            order_id=order_id,
            correlation_id=correlation_id,
            user_id=user_id,
            stage="notification_payment_failed_received",
            service="notification-service"
        )
        
        if not order_id:
            self.logger.warning(
                "🍕 ЗАКАЗ ПИЦЦЫ: PaymentFailed событие без order_id",
                correlation_id=correlation_id,
                stage="notification_payment_failed_validation_failed",
                service="notification-service",
                event_data=event_data
            )
            return
        ...
        self.logger.info(
            "🍕 ЗАКАЗ ПИЦЦЫ: Получаем шаблон уведомления для PaymentFailed",
            order_id=order_id,
            correlation_id=correlation_id,
            stage="notification_template_fetch",
            service="notification-service"
        ...
        if not template:
            self.logger.error(
                "🍕 ЗАКАЗ ПИЦЦЫ: Шаблон PaymentFailed не найден",
                order_id=order_id,
                correlation_id=correlation_id,
                stage="notification_template_not_found",
                service="notification-service"
        ...
        self.logger.info(
            "🍕 ЗАКАЗ ПИЦЦЫ: Формируем сообщение уведомления о неуспешном платеже",
            order_id=order_id,
            correlation_id=correlation_id,
            stage="notification_message_formatting",
            service="notification-service"
...
<<<<<<< HEAD
        ...
        if notification_id:
            self.logger.info(
                "🍕 ЗАКАЗ ПИЦЦЫ: Уведомление о неуспешном платеже обработано",
                order_id=order_id,
                correlation_id=correlation_id,
                notification_id=notification_id,
                stage="notification_payment_failed_completed",
                service="notification-service"
            )
        else:
            self.logger.error(
                "🍕 ЗАКАЗ ПИЦЦЫ: Ошибка при сохранении уведомления о неуспешном платеже",
                order_id=order_id,
                correlation_id=correlation_id,
